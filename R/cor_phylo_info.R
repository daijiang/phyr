...
check_phy <- function(phy) {
  if (!inherits(phy, "phylo")) {
    stop("\nIn the call to corphylo the input phylogeny is not of class \"phylo\".",
         call. = FALSE)
  }
  if (is.null(phy$edge.length)) {
    stop("\nIn the call to corphylo the input phylogeny has no branch lengths.",
         call. = FALSE)
  }
  if (is.null(phy$tip.label)) {
    stop("\nIn the call to corphylo the input phylogeny has no tip labels.",
         call. = FALSE)
  }
...
extract_species <- function(species, data, phy) {
  ...
  if (!length(species) %in% c(1, n)) {
    stop("\nThe species argument to corphylo must be a vector ",
  ...
  if (sum(duplicated(spp_vec)) > 0) {
    stop("\nDuplicate species not allowed in corphylo.", call. = FALSE)
  }
  if (!all(spp_vec %in% phy$tip.label)) {
    stop("\nIn corphylo, the `species` argument has one or more species not ",
    ...
    stop("\nIn corphylo, the phylogeny has one or more species not found ",
...
extract_matrices <- function(formula, data, phy, spp_vec) {
  ...
  if (!inherits(formula, "formula") || length(formula) != 3L) {
    stop("\nAll formulas in corphylo must be formula objects of one of the ",
  ...
  if (length(attr(terms(formula), 'term.labels')) == 1) {
    if (grepl("\\|", attr(terms(formula), 'term.labels'))) {
      ...
      if (grepl("(?!\\.)(?!_)[[:punct:]]", term_se[2], perl = TRUE)) {
        stop("\nThe following formula input to corphylo has >1 variable after ", 
             "the \"|\":",
      ...
      if (length(SeM) != n) {
        stop("\nIn the call to corphylo, the number of items in the ",
  ...
  if (nrow(U) != n) {
    stop("\nIn the call to corphylo, the number of rows in the ",

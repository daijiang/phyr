...
#'   (i.e., det(tree) != 1), `pglmm` will try to standarize it for you. 
...
#'   pglmm` will try to standarize it for you. No longer used: keep here for compatibility.
...
#' Vphy <- vcv(phy)
#' 
#' # Standardize the phylogenetic covariance matrix to have determinant = 1. (For an explanation of this standardization, see subsection 4.3.1 in Ives (2018))
#' Vphy <- Vphy/(det(Vphy)^(1/nspp))
#' 
#' # Construct the overall covariance matrix for phylogenetic attraction. (For an explanation of Kronecker products, see subsection 4.3.1 in the book)
...
#' 
#' # Display random effects: the function communityPGLMM.plot.re() does what the name implies. You can set show.image=T and show.sim.image=T to see the matrices and simulations, although below I have plotted them using the output from communityPGLMM.plot.re() to make figures appropriate for the book.
#' re <- pglmm.plot.re(Y ~ 1 + (1|sp__) + (1|site) + (1|sp__@site), data=d, cov_ranef = list(sp=phy), show.image=F, show.sim.image=F)
#' 
#' # The plots below are in black and white.
#' # Plot simulations of (1|site) and (1|sp) from communityPGLMM.plot.re()
#' layout(matrix(c(1,2), nrow=1))
#' par(mai=c(1,1,1,.1))
#' image(t(re$sim$"1|site"[nsite:1,]), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|site)", col=gray(.01*(1:99)))
#' mtext(side=1, "sp", padj = .5, cex=1)
#' mtext(side=2, "site", padj = -.5, cex=1)
#' image(t(re$sim$"1|sp"[nsite:1,]), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|sp)", col=gray(.01*(1:100)))
#' mtext(side=1, "sp", padj = .5, cex=1)
#' mtext(side=2, "site", padj = -.5, cex=1)
#' 
#' # Plot the covariance matrices of (1|site) and (1|sp) from communityPGLMM.plot.re()
#' layout(matrix(c(1,2), nrow=1))
#' par(mai=c(.5,.5,.5,.1))
#' V <- re$vcv$'1|site'[(nspp*nsite):1,]
#' image(t(V), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|site)", col=gray(.01*(1:100)))
#' V <- re$vcv$'1|sp'[(nspp*nsite):1,]
#' image(t(V), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|sp)", col=gray(.01*(1:100)))
#' 
#' # Plot simulations of (1|sp__@site) and (1|sp) with phylogeny
#' layout(matrix(c(1,2,2,3,4,4), ncol=2))
#' par(mai=c(0,.7,.5,.3))
#' plot(phy, direction="downwards", cex=.01, main="(1|sp__)",cex.main=1.5)
#' par(mai=c(1,.5,0,.1))
#' image(t(re$sim$'1|sp__'), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "", col=gray(.01*(1:100)))
#' mtext(side=1, "sp", padj = .5, cex=1)
#' mtext(side=2, "site", padj = -.5, cex=1)
#' 
#' par(mai=c(0,.7,.5,.3))
#' plot(phy, direction="downwards", cex=.01, main="(1|sp__@site)",cex.main=1.5)
#' par(mai=c(1,.5,0,.1))
#' image(t(re$sim$'1|sp__@site'), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "", col=gray(.01*(1:100)))
#' mtext(side=1, "sp", padj = .5, cex=1)
#' mtext(side=2, "site", padj = -.5, cex=1)
#' 
#' # Plot the covariance matrices of (1|sp__) and (1|sp__@site) from communityPGLMM.plot.re()
#' layout(matrix(c(1,2), nrow=1))
#' par(mai=c(.5,.5,.5,.1))
#' V <- re$vcv$'1|sp__'[(nspp*nsite):1,]
#' image(t(V), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|sp__)", col=gray(.01*(1:100)))
#' V <- re$vcv$'1|sp__@site'[(nspp*nsite):1,]
#' image(t(V), xaxt="n", yaxt="n", ylab = "", xlab = "", main = "(1|sp__@site)", col=gray(.01*(1:100)))
#' 
#' #################################################
...
#' 
#' # Construct the dataset
#' d <- data.frame(sp=rep(phy.sp$tip.label, times=nsite), site=rep(phy.site$tip.label, each=nspp))
#' 
#' # Simulate abundance data
...
#' # Note that, if show.image and show.sim are not specified, pglmm.plot.re() shows the covariance matrices if nspp * nsite < 200 and shows simulations #' if nspp * nsite > 100
#' re <- pglmm.plot.re(Y ~ 1 + (1|sp__) + (1|site__) + (1|sp__@site) + (1|sp@site__) + (1|sp__@site__),  
...
#'                         + (1|sp@site__) 
#'                         + (1|sp__@site__), 
#'                         data=d, cov_ranef=list(sp=phy.sp, site=phy.site), s2.init=c(mod.r$ss, 0.01)^2)
#' mod.f

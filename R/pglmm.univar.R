...
#' 
#' `pglmm.univar` performs linear regression for binomial and Poisson phylogenetic data,
...
#' for non-Gaussian data. The method uses a GLMM approach, alternating between
...
#' of the model.
#' 
#' `pglmm.univar` in the package `phyr` is similar to `binaryPGLMM` in 
#' the package `ape`, although accepts binomial and Poisson data, computes the 
#' log likelihood, and also returns the necessary information for computing
#' a phylogenetic R2 in the package `rr2`. `pglmm.univar` differs from `pglmm`
#' because it is designed for univariate data common in phylogenetic comparative
#' studies.
#' 
...
#' for binomial and Poisson data. The standard errors of B are computed from
#' the PQL results conditional on the estimate of s2 and therefore should tend 
#' to be too small. The function returns an approximate P-value for the hypothesis 
#' of no phylogenetic signal in the residuals (i.e., H0:s2 = 0) using an approximate
...
#' @param family Either "binomial" for binomial dependent data, or "poisson" for
#'   count data. It should be specified as a character string (i.e., quoted). 
#'   For binomial data, the link function is logit; for poisson data,
...
#' 
#' @return An object of class "pglmm.univar".
#' 
...
#' 
#' \dontrun{
#' ## Illustration of `pglmm.univar` with simulated data
#' 
...
#' 
#' # Fit model
#' pglmm.univar(Y ~ X1, method="binomial", phy=phy, data=sim.dat)
#' 
...
#' 
#' ## Examine bias in estimates of B1 and s2 from `pglmm.univar` with
...
#'       sim <- ape::binaryPGLMM.sim(Y ~ 1 + X, phy = phy, data = sim.dat, s2 = s2,
#'                                        B = matrix(c(0,B1), nrow = 2, ncol = 1), nrep = 1)
#'       sim.dat$Y <- sim$Y
#' 
#'       z <- pglmm.univar(Y ~ 1 + X, method="binomial", phy = phy, data = sim.dat)
#' 
...
#' meanEsts
#' 
#' ## Examine type I errors for estimates of B0 and s2 from `pglmm.univar`
...
#' 
#'     sim.dat <- data.frame(Y = array(0, dim = n), X = X, row.names = phy$tip.label)
#'     sim <- ape::binaryPGLMM.sim(Y ~ 1, phy = phy, data = sim.dat, s2 = s2,
#'                            B = matrix(B0, nrow = 1, ncol = 1), nrep = 1)
#'     sim.dat$Y <- sim$Y
#' 
#'     z <- pglmm.univar(Y ~ 1, phy = phy, data = sim.dat)
#' 
...
#' # used to simulate the data. These results show that `pglmm.univar` tends to
...
#' }
#'
pglmm.univar <- function(formula, family = "gaussian", 
                         data = list(), 
                         phy, 
                         REML = TRUE, 
  ...
  # re.1 <- list(1, sp = as.factor(data$sp), covar = vcv(phy))
  ...
  class(results) <- "pglmm.univar"
  results
}

#' Print summary information of fitted `pglmm.univar` model
#' 
#' @method print pglmm.univar
#' @param x An object of class "pglmm.univar".
...
#' @rdname pglmm.univar
#' 
print.pglmm.univar <- function (x, digits = max(4, getOption("digits") - 4), ...) {
  cat("\n\nCall:")
  print(x$formula)
  cat("\n")

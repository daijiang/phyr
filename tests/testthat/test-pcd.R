...
test_that("testing pcd_pred, which calculate expectation of conditional PSV", {
  ...
  x2 = pcd_pred(comm_a, comm_b, tree = phylotree, reps = 100, cpp = F)
  expect_type(x1, "list")
  expect_length(x1, 4)
  expect_type(x2, "list")
  expect_length(x2, 4)
...
test_that("testing pcd, which calculate pairwise site dissimilarity", {
  ...
  x4 = pcd(comm = comm_a, tree = phylotree, expectation = x1, cpp = F)
  expect_type(x3, "list")
  expect_type(x4, "list")
...
test_that("testing pcd, without provide expectation", {
  ...
  x6 = pcd(comm = comm_a, tree = phylotree, cpp = F)
  expect_type(x5, "list")
  expect_type(x6, "list")
})
...
test_that("testing pcd, expectation based on one community", {
  ...
  x8 = pcd(comm = comm_a, tree = phylotree, expectation = x1, cpp = F)

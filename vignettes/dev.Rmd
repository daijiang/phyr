...
d$p <- d$y > 0
library(lme4)
z <- glmer(p ~ x1 + (1 | sp), data=d, family="binomial")
library(phyr)
mod <- communityPGLMM(p ~ x1 + (1 | sp), data=d, tree = phy, REML=F, reltol=10^-10, family="binomial")
summary(z)

# predictions
pred_lmer <- predict(z)
logit = make.link("logit")$linkfun
plot(logit(fitted(z)), pred_lmer) # same
pred_phyr <- communityPGLMM.predicted.values(mod)$Y_hat # XB + b
plot(pred_lmer, pred_phyr) # good
...
plot(resid(z), (d$p - hat_lmer)/sqrt(hat_lmer * (1 - hat_lmer))) # hum...
plot(mod$mu, fitted(z)) # same

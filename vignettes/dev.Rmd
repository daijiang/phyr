...
d$p <- d$y > 0
z_lmer <- glmer(p ~ x1 + (1 | sp) + (1|site), data=d, family="binomial")
z_phyr <- communityPGLMM(p ~ x1 + (1 | sp) + (1|site), data=d, REML=F, family="binomial")
summary(z_lmer)
z_phyr
...
# residuals
plot(residuals(z_lmer, type = "response"), 
     residuals(z_phyr, type = "response"))

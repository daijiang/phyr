...
The main function to use is `phyr::communityPGLMM.plot.re()` (alias: `phyr::communityPGLMM.show.re()`). Here are the arguments of this function:

```{r args-pglmm}
args(phyr::communityPGLMM.plot.re)
```
...
- `add.tree.site` (`TRUE` or `FALSE`): when `show.sim.image = TRUE`, whether to add a phylogeny of sites at the right of each matrix plot? This can be usefull for bipartite problems (e.g. pollinators (species) and plants (sites)).
...
mod1re = communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = TRUE, show.sim.image = FALSE)
```

In the above plot, we can see that some of the panels are black-white but some have colors. This is because, by default, if a mtrix has both positive and negative values, then the function will use red-blue color and will draw a key for that (use `colorkey = FALSE` to suppress it). If a matrix does not have negative values, then the function will use black/white color (use `useAbs = FALSE` to use color instead, and use `colorkey = FALSE` to suppress key if wanted). In both cases, value 0 will be white so that the structure of the var-cov matrix can be easier to see.

```{r fig.asp=0.6}
# all use color with useAbs = FALSE
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = TRUE, show.sim.image = FALSE, useAbs = FALSE)
```

For the above plot, notice that for `1|sp` and `1|site`, all values are either 1 or 0 even though we have a range in the key. We can supress the key with `colorkey = FALSE`.

```{r fig.asp=0.6}
# suppress key with colorkey = FALSE
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = TRUE, show.sim.image = FALSE, useAbs = FALSE, colorkey = FALSE)
```
...
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = TRUE, show.sim.image = FALSE, colorkey = FALSE)
```
...
# all black and white
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = TRUE, show.sim.image = FALSE, useAbs = TRUE)
```
...
To save this customed plot, we can wrap the above line of code within `pdf()` and `dev.off()`.
...
mod1sim = communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = FALSE, show.sim.image = TRUE)
```
...
```{r fig.asp=0.6}
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = FALSE, show.sim.image = TRUE, add.tree.sp = FALSE)
```
...
```{r fig.asp=0.6}
communityPGLMM.plot.re(Y ~ 1 + env + (1|sp__) + (1|site) + (env|sp__) + (1|sp__@site),
                       data = d, tree = phy, show.image = FALSE, show.sim.image = TRUE, add.tree.sp = TRUE,
...
```{r fig.asp=0.6}
communityPGLMM.plot.re(x = mod_1, show.image = FALSE, show.sim.image = TRUE, add.tree.sp = TRUE,
                       colorkey = FALSE, useAbs = FALSE)
